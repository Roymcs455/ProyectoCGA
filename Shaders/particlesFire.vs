...
vec3 randomInitialVelocity() {
    float velocity = mix(2.1, 4.5, texelFetch(RandomTex, 2 * gl_VertexID, 0).r ); //Velocidad y alto
    return EmitterBasis * vec3(0, velocity, 0);
}

vec3 randomInitialPosition() {
    float offset = mix(-0.5, 0.5, texelFetch(RandomTex, 2 * gl_VertexID + 1, 0).r); //Ancho
